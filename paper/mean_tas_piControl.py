"""
piControl reference comparison: full-run mean vs. legacy 31-yr branch window.
...
def _branch_reference(model, ru_path, pi_years, pi_gmsat, bi):
    """Return (31-yr branch mean, note) where note is set when the mean is unavailable.

    ``resolve_branch_year`` no longer raises for an out-of-span branch year (it
    is a warning under the full-mean baseline; see baseline.py), and
    ``legacy_window_reference`` silently falls back to the control's first
    ``BRANCH_WINDOW`` years whenever the centred window would start before the
    control record begins -- which triggers for a branch predating the entire
    control, not just a true day-0 branch. That fallback is a real number, but
    it is not testing the same centred-window definition as for other models,
    so we check the span explicitly here rather than relying on either
    function to flag it.
    """
    if ru_path is None:
        return np.nan, "no ramp-up tas"
    if bi.year is None:
        return np.nan, "missing branch year"
    ...
    try:
        ...
        if not (pi_years.min() <= branch <= pi_years.max()):
            return np.nan, "branch year out of range"
        ref = legacy_window_reference(
            pi_years,
            pi_gmsat,
            branch,
...
def main(up2p0_dir, picontrol_dir, out_path=None):
    ...
    for model in sorted(pi_files):
        ...
        if ru_path is None:
            ...
            ref, note = _branch_reference(model, ru_path, pi_years, pi_gmsat, bi)
            branch_means.append(ref)
            branch_notes.append(note)

        with xr.open_dataset(pi_path, decode_times=False) as ds:
            model_names.append(str(ds.attrs.get("source_id", model)))
    ...
    gs = GridSpec(1, 4, figure=fig, width_ratios=[0.65, 1.15, 0.85, 0.85], wspace=0.05)
    ...
    for i, (name, branch_year) in enumerate(zip(model_names, branch_years)):
        ...
        branch_label = f"branch {branch_year}" if branch_year is not None else "branch —"
        ax_names.text(
            1.0,
            i - 0.12,
            name,
    ...
    ax_means.set_xlabel("Mean tas (K)", fontsize=10)
    ...
    ax_diff.set_xlabel("Δ tas (K)", fontsize=10)
    ax_diff.set_title("Full − 31-yr branch", fontsize=11, pad=8)
    ax_diff.set_xlim(-0.12, 0.12)

    ax_drift.scatter(drifts, y, color="C3", s=42, edgecolor="k", linewidth=0.6, zorder=3)
