...
PI_DIR = Path("/Users/jakobharteg/Desktop/tipmip/tas/esm-piControl/gmstmon/")
UP_DIR = Path("/Users/jakobharteg/Desktop/tipmip/tas/esm-up2p0/gmstmon/")
BRANCH_WINDOW = 31
ROW_OFFSET = 0.17  # vertical offset between the two dots within each row

pi_files = discover(PI_DIR)
ru_files = discover(UP_DIR)
...
def _branch_reference(model, ru_path, pi_years, pi_gmsat, bi):
    ...
    try:
        ...
        msg = str(exc)
        if "outside piControl span" in msg:
            note = "branch year out of range"
        elif "could not be decoded" in msg:
            note = "missing branch year"
        elif "not contained in piControl" in msg or "no piControl data" in msg:
...
gs = GridSpec(1, 4, figure=fig, width_ratios=[0.85, 1.15, 0.85, 0.85], wspace=0.05)
